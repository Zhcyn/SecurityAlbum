...
#import "HHSQLiteManager.h"
#import "HHAlbumModel.h"
#import "HHPhotoModel.h"
#import <FMDB/FMDB.h>

@implementation HHSQLiteManager

#pragma mark - Public

+ (instancetype)sharedSQLiteManager {
    static HHSQLiteManager *manager = nil;
...
/**
 ...
 @return 相册数组
 */
- (NSMutableArray<HHAlbumModel *> *)requestUserAlbums {
    ...
    for (HHAlbumModel *album in albums) {
        // 获取相册下最新添加的图片作为相册缩略图
        ...
        HHPhotoModel *thumbPhoto = [[self photosWithResultSet:thumbSet] firstObject];
...
/**
 ...
 @return 新创建的相册
 */
- (HHAlbumModel *)createAlbumWithName:(NSString *)name {
    FMDatabase *db = [FMDatabase databaseWithPath:[self databaseFilePath]];
...
/**
 ...
 @return 是否删除成功
 */
- (BOOL)deleteAlbumWithAlbum:(HHAlbumModel *)album{
 ...
 */
- (BOOL)resortAlbums:(NSArray<HHAlbumModel *> *)sortAlbums {
    if (0 == sortAlbums.count) return NO;
    FMDatabase *db = [FMDatabase databaseWithPath:[self databaseFilePath]];
    if (![db open]) return NO;
    [db beginTransaction];
    [sortAlbums enumerateObjectsUsingBlock:^(HHAlbumModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
/**
 ...
 @return BOOL
 */
- (BOOL)addPhotos:(NSArray<HHPhotoModel *> *)photos {
    ...
    for (HHPhotoModel *photo in photos) {
...
/**
 ...
 @return 指定页码对应的图片数据
 */
- (NSMutableArray<HHPhotoModel *> *)requestPhotosWithAlbumid:(NSInteger)albumid page:(NSInteger)page pagesize:(NSInteger)pagesize {
    FMDatabase *db = [FMDatabase databaseWithPath:[self databaseFilePath]];
    if (![db open]) return nil;
    FMResultSet *set = [db executeQuery:@"SELECT * FROM photos WHERE albumid=? LIMIT ? OFFSET ?", @(albumid),@(pagesize),@(MAX(0, page-1)*pagesize)];
    NSMutableArray<HHPhotoModel *> *photos = [self photosWithResultSet:set];
    [db close];
    return photos;
}
...
/**
 ...
 @return 该相册下的所有图片数据
 */
- (NSMutableArray<HHPhotoModel *> *)requestAllPhotosWithAlbumid:(NSInteger)albumid {
...
/**
 ...
 @return 多张图片信息
 */
- (NSArray<HHPhotoModel *> *)requestLatestPhotosWithAlbumid:(NSInteger)albumid count:(NSInteger)count {
    FMDatabase *db = [FMDatabase databaseWithPath:[self databaseFilePath]];
    if (![db open]) return nil;
...
/**
 ...
 @return 成功删除图片的个数
 */
- (NSUInteger)deletePhotos:(NSArray<HHPhotoModel *> *)photos fromAlbum:(HHAlbumModel *)album {
    if (0 == photos.count || nil == album) return 0;
    FMDatabase *db = [FMDatabase databaseWithPath:[self databaseFilePath]];
    if (![db open]) return 0;
    NSMutableArray<HHPhotoModel *> *removePhotos = [NSMutableArray array];
    // 开启事务,删除数据库中图片的数据
    [db beginTransaction];
    for (HHPhotoModel *photo in photos) {
    ...
    for (HHPhotoModel *photo in removePhotos) {
...
/**
 ...
 @return 相册模型数组
 */
- (NSMutableArray<HHAlbumModel *> *)albumsWithResultSet:(FMResultSet *)set {
    NSMutableArray<HHAlbumModel *> *albums = [NSMutableArray array];
    while ([set next]) {
        HHAlbumModel *album = [[HHAlbumModel alloc] init];
...
/**
 ...
 @return 图片模型数组
 */
- (NSMutableArray<HHPhotoModel *> *)photosWithResultSet:(FMResultSet *)set {
