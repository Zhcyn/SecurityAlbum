...
+(NSString *)getDateStringFromMilliSeconds:(long long)miliSeconds{
    if (miliSeconds<0) {
        return @"";
    }
    NSDate *date = [NSDate getDateTimeFromMilliSeconds:miliSeconds];
    NSString *dateStr = [date LocalTimeISO8601String];
    
    NSTimeInterval currentInterval = [NSDate getDateSecond:[[NSDate date]LocalTimeISO8601String]]*1000.0;
    
    if (currentInterval-miliSeconds<24*60*60*1000) {//一天内
        if (dateStr.length>11) {
            return [dateStr substringFromIndex:11];
        } else {
            return @"";
        }
    } else if (currentInterval-miliSeconds<7*24*60*60*1000) {//一周内
        return [NSString stringWithFormat:@"Week%@",[self weekWithTime:dateStr]];
    } else {//多于一周
...
+(NSString*)getLoadingTitleFromTime:(NSString*)startLoadTime
{
    ...
    {
        return @"There is a little too much data, please wait a moment...";
    }
    else if(seconds<=90)
    {
        return @"Keep trying to synchronize...";
    }
    else if(seconds<=120)
    {
        return @"Haven't uploaded for a few days？";
    }
    else if(seconds<=150)
    {
        return @"That's a lot of data...";
    }
    else if(seconds<=180)
    {
        return @"It will be over soon...";
    }
    else{
        return @"Victory is just around the corner. Hold on...";
    }
    return @"";
}
